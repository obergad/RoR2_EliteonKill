...
namespace ExamplePlugin
{
    ...
    public class becomeelite : BaseUnityPlugin
	{
        ...
        public void Awake()
        {
            ...
            myItemDef.name = "EXAMPLE_ELITEONKILLL_NAME";
            myItemDef.nameToken = "EXAMPLE_ELITEONKILL_NAME";
            myItemDef.pickupToken = "EXAMPLE_ELITEONKILL_PICKUP";
            myItemDef.descriptionToken = "EXAMPLE_ELITEONKILL_DESC";
            myItemDef.loreToken = "EXAMPLE_ELITEONKILL_LORE";
        ...
        private void GlobalEventManager_onCharacterDeathGlobal(DamageReport report)
        {
            ...
            if (attacker.inventory)
            {
                    ...
                    Util.CheckRoll(5, attacker.master))
                {
                    ...
                    int num = rnd.Next(6);  // creates a number between 0 and 6 
                    if(num == 0) { attacker.AddTimedBuff(RoR2Content.Buffs.AffixRed, 5 + garbCount, 1); }
                    else if(num == 1) { attacker.AddTimedBuff(RoR2Content.Buffs.AffixBlue, 5 + garbCount, 1); }
                    else if(num == 2) { attacker.AddTimedBuff(RoR2Content.Buffs.AffixWhite, 5 + garbCount, 1); }
                    else if(num == 3) { attacker.AddTimedBuff(RoR2Content.Buffs.AffixPoison, 5 + garbCount, 1); }
                    else if(num == 4) { attacker.AddTimedBuff(RoR2Content.Buffs.AffixHaunted, 5 + garbCount, 1); }
                    else if (num == 5) { attacker.AddTimedBuff(RoR2Content.Buffs.AffixLunar, 5 + garbCount, 1); }
                }
            }
        }
        ...
        private void AddTokens()
        {
            ...
            LanguageAPI.Add("EXAMPLE_ELITEONKILL_NAME", "?");

            //The Pickup is the short text that appears when you first pick this up. This text should be short and to the point, numbers are generally ommited.
            LanguageAPI.Add("EXAMPLE_ELITEONKILLL_PICKUP", "?");

            //The Description is where you put the actual numbers and give an advanced description.
            LanguageAPI.Add("EXAMPLE_ELITEONKILL_DESC", "Whenever you <style=cIsDamage>kill an enemy</style>, you have a <style=cIsUtility>5%</style> chance to become a random elite for <style=cIsUtility>5s</style> <style=cStack>(+1s per stack)</style>.");
            
            //The Lore is, well, flavor. You can write pretty much whatever you want here.
            LanguageAPI.Add("EXAMPLE_ELITEONKILL_LORE", "?");
        }
